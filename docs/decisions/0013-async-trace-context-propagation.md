...
- Empty context — tracing disabled, i.e. no `OTEL_EXPORTER_OTLP_ENDPOINT` configured — degrades gracefully: `Inject` writes nothing, the carrier stays empty, the row gets `{}` headers and `""` traceparent, which is byte-identical to pre-M5 behavior. Every existing outbox and E2E test keeps passing unchanged, confirmed by running `make test-e2e` against a core-only stack (no observability containers).
