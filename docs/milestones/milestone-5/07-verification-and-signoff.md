...
- **⚠️ TRACKED FOLLOW-UP — `scripts/verify-trace.sh` can print a spurious `FAIL` even when the trace chain is intact.** The script queries Jaeger for the single most recent `wallet-service` trace (`limit=1`, ordered by recency — Step 2). Unlike the Go services, which deliberately exclude `/health` and `/metrics` from tracing (Task 03's brief; see `services/ledger-service/internal/handler/routes.go` for the pattern), wallet-service's `/health/live` endpoint *is* traced, and Docker hits it every 10 seconds. When a health-check trace lands more recently than a just-made transfer trace, the script picks the health-check trace, finds it missing the `ledger-service`/`projection-service` spans, and prints `FAIL: the end-to-end trace is broken — NFR-OBS-5 not satisfied` — a false negative with an actively misleading message, since the actual transfer's trace chain is intact. See the Step 2 finding in the Implementation Record below for the observed detail. This is **not fixed in this milestone**: the brief mandated the script verbatim, the real fix (scoping wallet-service instrumentation to exclude `/health/live`, matching the Go services' pattern) is Task 04's territory and out of this gate task's scope, and the script isn't wired into any Makefile target or CI workflow today, so nothing is actually flaky yet. **It must be resolved — either by excluding `/health/live` from wallet-service tracing or by having the script filter to multi-service traces — before `verify-trace.sh` is ever wired into a Makefile target or CI workflow.**
